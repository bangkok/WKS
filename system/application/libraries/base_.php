...
class Base1 extends Controller
{
    ...
    function _log_check()
    {
        ...
        && $this->auth_model->isUser($user["mail"], $user["pass"]))
        {
            $this->data["authed"] = true;
            $this->default_role = $groups = $this->auth_model->getUserGroups($user["mail"]);
            $this->data['auth']["mail"] = $user["mail"];
            $this->data['auth']["userData"] = $this->auth_model->getUser($user['mail']);
...
*/
  ...
  /***************************************
    ...
    function accessDeny()
    {
        ...
        echo "Доступ <strong>",$user["username"],"</strong> к странице запрещен.";
        exit;
    }
