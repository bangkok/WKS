<?
@include_once("base.php");
class Typical_projects extends Base {

	function __construct()
	{
		parent::Base();	


		
		//print_r($this->data['Menu']);
		//$this->data['Content']['text']=$this->config_model->getText($this->data['Content']['id'],'about');	
	}
	
	function _remap()
    {



			if($link = $this->uri->segment(3)){
				if(method_exists($this, $link)) $this->$link($this->uri->segment(4),$this->uri->segment(5));
				else $this->index($this->uri->segment(3),$this->uri->segment(4));
			}
			else $this->index($this->uri->segment(3),$this->uri->segment(4));
		
    
     /*
     
      if(eregi('symbol',$this->uri->uri_string()))
           $this->symbol($link,$this->uri->segment($i-1),$this->uri->segment(3));
      elseif(eregi('stat',$this->uri->uri_string()))
           $this->stat($link,$this->uri->segment($i-1),$this->uri->segment(3));
      else $this->index($link,$this->uri->segment($i-1),$this->uri->segment(3));
     */
    }

	function index($sector=0, $page)
	{//$sector = 2;
		$this->load->model('viewconfig_model');
    		$this->load->library('pagination');


	if(isset($sector) && !empty($sector)){
		$this->load->model('catalog_model');
		$this->data['Catalog']['TMap'] = $this->catalog_model->getCatalog();
		//print_r($this->data['Catalog']['TMap']);

$this->data['cnt'] = $this->catalog_model->getCntHouses($sector);
$this->data['per_page']=$this->config_model->getConfigName('num_houses');
		$this->data['Catalog']['Houses'] = $this->catalog_model->getCompanys($sector, $page);
		$this->data['Content']['name'] = $this->data['Catalog']['name'];

		//print_r($this->data['Catalog']['Houses']);


      $config=$this->viewconfig_model->getNumPage(base_url().$this->uri->segment(1)."/".$this->uri->segment(2)."/".$this->uri->segment(3)."/",4,$this->data['cnt'],$this->data['per_page']);
      $this->pagination->initialize($config);
      $this->data['Catalog']['links'] = $this->pagination->create_links();

//print_r($this->data['links']);

		$this->data['Styles'] .= $this->AddStyle('button'); 
		$this->data['Content']['text'] = $this->load->view($this->data['papka'].'/catalog.php',$this->data['Catalog'], true);
	}
	else {
		$this->data['template_file_path'] = "tpl/list_page.tpl";
		$this->data['Styles'] .= $this->AddStyle('home');
		//$this->data['Styles'] .= $this->AddStyle('style_WKS_projects');

		}
     		$this->load->view('pageconstructor', $this->data);
	}

	function house($link, $show='')
	{
	if(!$link){ header("Location: /construction/typical_projects"); return 0;}


		$this->data['Styles'] .= $this->AddStyle('style_WKS_house'); 

		$this->load->model('catalog_model');
		$this->data['House'] = $this->catalog_model->getHouse($link);

	//print_r($this->data['House'][0]);

		
		$this->data['Content']['text'] = $this->load->view($this->data['papka'].'/house.php',$this->data['House'][0], true);
	
		$this->data['Content']['name'] = $this->data['House'][0]->title;
     		$this->load->view('pageconstructor', $this->data);
	}

	function show($house, $img='')
	{

	$this->data['Styles'] .= $this->AddStyle('basic- | galleriffic-4');
	$this->data['Js'] .= $this->AddJs('jquery.galleriffic | jquery.opacityrollover');

		if(!$house || !is_numeric($house)){ header("Location: /construction/typical_projects"); return 0;}
		$this->load->model('catalog_model');
		$this->data['House'] = $this->catalog_model->getHouse($house);


		$this->data['Content']['name'] = $this->data['House'][0]->title;
		$this->data['Content']['text'] = $this->load->view($this->data['papka'].'/house-gallery.php',$this->data['House'][0], true);

     		$this->load->view('pageconstructor', $this->data);
	}

function	GetFile($path_file)
{	if(file_exists($path_file)){
		$handle_file	= fopen($path_file, "r");
		$text	= fread ($handle_file, filesize ($path_file));
		fclose($handle_file);}
return	$text;
}

}
?>