...
//			array ("link" => "http://".$_SERVER['HTTP_HOST']."/dfg/", "text" => "поиск адреса", "title" => ""),
	...
	$termLinks = array(
			/*array (
				"terms" => array("дом"),
			 	"link" => "http://".$_SERVER['HTTP_HOST']."/objects/", 
			 	"text" => "объекты", 
			 	"title" => ""),
			array (
				"terms" => array("дом2"),
			 	"link" => "http://".$_SERVER['HTTP_HOST']."/objects2/", 
			 	"text" => "объекты2", 
			 	"title" => "")*/			
		);
...
else
{
	...
	$config["A_SMTP_user"] = "";
	$config["A_SMTP_pass"] = "";
	//$config["A_admin_mail"] = "sichevoy@gmail.com";
	$config["A_admin_mail"] = "box4list@ya.ru";
	$config["A_admin_mail_CC"] =array(
	...
	$helpSearchLinks = array(
//			array ("link" => "http://".$_SERVER['HTTP_HOST']."/dgdfg/", "text" => "поиск адреса", "title" => ""),
	);
...
//		if (
//			stripos(urldecode($strSearchTerms), "дом") != ""
//			|| stripos(urldecode($strSearchTerms), "улица") != ""
//		)
//		{
//			//echo "<a href='http://".$_SERVER['HTTP_HOST']."/objects/'>объекты</a><br />";
//			foreach ($termLinks AS $val)
//			{
//				echo "<a href='".$val["link"]."' title=\"".$val["title"]."\">".$val["text"]."</a><br />";
//			}			
...
/*
		...
		{
			$str .= '<p>Похоже ссылка на нашем сайте неверна - мы просим прощения, '
				.'мы гарантируем что проверим эту ссылку и исправим ее. '
				.'Все что вам необходимо сделать это нажать нижеследующую кнопку.</p>';
		}
		else
		{
			$str .= '<p>Для улучшения нашего сервиса вы можете информировать нас о том что имеется '
				.'неверная ссылка на наш сайт. Мы сообщим о том что страница перемещена'
				.' и необходимо обновить эту ссылку. '
				.'<strong>Только нажмите кнопку!</strong></p>';
		}
*/
		//$str .= '<form method="post" action="http://'.$_SERVER['HTTP_HOST'].'/feed404/" onsubmit="alert(\'Спасибо что вы дали нам знать!\');">';
		//$str .= '<input type="hidden" name="activenumber" value="1111" \/>';
		//$str .= '<input type="hidden" name="username" value="accessify" \/>';
		//$str .= 'Неверная ссылка на страницу - ['.$_SERVER['HTTP_HOST'].'] reffer - ['.$strReferrer.']';
		//$str .= '</div>';
		$txt = '';
