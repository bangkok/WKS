...
class Publication_model extends Model
{
   function __construct()
     ...
     */
    function getPublic($table, $col = 0, $from = 0,$lang = 'all')
    {//echo "!!!!!!!!!!!!!!!!!!!!!!!$from<br>";echo "!!!!!!!!!!!!!!!!!!!!!!!$col<br>";
        $this->db->orderby('date', 'desc');
        $this->db->select('*, date_format(date, \'%d.%m.%Y\') as dates');
    ...
    function getCnt($table,$lang='all')
    {
 ...
 function  getPublicById($id,$table,$lang)
    {
        $this->db->where('id', $id);
        $this->db->where('sleep', 'y');
        if($lang!='no'&&$lang!='all')
 ...
 function getPublicBlock($table,$lang,$kol){
